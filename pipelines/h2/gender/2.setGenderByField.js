...
const buildPipeline = (genderKeywords, field) => {
    let conditions = [];
    ...
    for (const [gender, keywords] of Object.entries(genderKeywords)) {
        ...
        keywords.forEach(keyword => {
            regexConditions.push({ "$regexMatch": { "input": `$${field}`, "regex": keyword, "options": "i" } });
...
function setGenderByField({ id,field }) {

    const genderKeywords = JSON.parse(fs.readFileSync(`${process.cwd()}/pipelines/h2/gender/_gender_keywords-${id}.json`, 'utf8'));

    const pipeline = buildPipeline(genderKeywords,field);

    return pipeline
}
export default setGenderByField;